...
class TTDiagnosis:
    ...
    CMD_FAULT_STATUS = 0x0501
    CMD_GW_MNGR_LIST = 0x0E02
    GW_SOCKET_FILE = "/tmp/ttgw.socket"
    PING_URL       = "nmcheck.gnome.org"
    ...
    @property
    def diagnosis_url(self):
        diagnosis_url = f"{self.url}/data/gw-diagnostic/"

        return diagnosis_url

    def __init__(self):
        if not config.is_loaded():
            config.read()
        self.http_helper = HttpHelper(self.url, self.user, self.password)
        self.diagnosis = {
            "general": {
                "hostname": "",
                "localtime": "",
    ...
    def gw_send_command(self, cmd, params=None):
        ...
        try:
            ...
            logger.error("Broken pipe while sending a gateway cmd")
            return None
        raw_data = gw_socket.recv(1024)
        if not raw_data:
            logger.error("Server ended connection while sending a gateway cmd")
            return None
        ...
        while len(data) < data_len:
            raw_data = gw_socket.recv(1024)
            if not raw_data:
                logger.error("Server ended connection while receiving a " +
                        "gateway cmd response")
                return None
            data += raw_data
        gw_socket.close()
    ...
    async def get_iface_info(self, iface: str):
        ...
        ping = await self.get_ping(iface)
    ...
    async def get_ping(self, iface=None):
        """Returns ping to TycheTools ECOaaS info"""
        response = rtt = None
        if iface is None:
            retval, out = await self.shell(f"ping -q -c3 -W5 {self.PING_URL}")
        else:
            retval, out = await self.shell(f"ping -I {iface} " + \
                f"-q -c3 -W5 {self.PING_URL}")
        if retval != 0 or out is None:
            response = False
        else:
            response = True
            match = re.search(".*=\s[\d\.]+\/([\d\.]+)", out)
            if match:
    ...
    async def get_fault_status(self):
        """Returns gateway info"""
        status = strategy = transport = state = cluster = leader = None
        resp = self.gw_send_command(self.CMD_FAULT_STATUS)
        if resp is not None:
            init = resp["success"]
            if init:
                data = resp["data"]
    ...
    async def get_gw_manager_info(self):
        """Returns gateway manager list"""
        role = remote_host = server_port = active_gw = id_count = None
        gw_local = gw_remote = []
        resp = self.gw_send_command(self.CMD_GW_MNGR_LIST)
        if resp is not None:
            init = resp["success"]
            if init:
                data = resp["data"]["data"]
                ...
                active_gw = data["active_gw"]
                id_count = data["id_count"]
                gw_local = data["gw_local"]
                for loc in gw_local:
                    loc.pop("whitelist", None)
                gw_remote = data["gw_remote"]
                for rem in gw_remote:
                    rem.pop("whitelist", None)
        return {
            "role": role,
    ...
    async def run(self):
        ...
        devices, iface_en, iface_type = await self.get_ifaces()
        self.diagnosis["interfaces"] = []
        for dev in devices:
            iface = await self.get_iface_info(dev)
            interface = {}
            interface["name"] = dev
        ...
        if iface_en:
            ...
            iface_en_txrx = await self.get_iface_txrx(iface_en)
        ...
        if iface_type == "wifi":
            ...
            wifi_info = await self.get_wifi_info()
        ...
        if iface_type == "gsm":
            gsm_info = await self.get_gsm_info()
        ...
        self.diagnosis["general"]["hostname"] =  await self.get_hostname()
        self.diagnosis["general"]["localtime"] = await self.get_date()
        self.diagnosis["general"]["uptime_s"] = await self.get_uptime()
        ttversion = await self.get_ttversion()
        ...
        self.diagnosis["general"]["bt_mac"] = await self.get_bt_mac()
        ...
        self.diagnosis["general"]["public_ip"] = await self.get_public_ip()
        gw_info = await self.get_gw_info()
        cvg_info = await self.get_node_coverage()
        fault_info = await self.get_fault_status()
        gw_manager_info = await self.get_gw_manager_info()
        ...
        self.diagnosis["proccesses"]["gateway"]["coverage"] = \
            cvg_info["coverage"]
        pip_ver = await self.get_pip_ver()
        self.diagnosis["proccesses"]["ble_config"]["version"] = \
            pip_ver["bleconf"]
        self.diagnosis["proccesses"]["ble_config"]["status"] = \
            await self.get_ble_status()
        gw_apps = await self.get_gw_apps()
        self.diagnosis["proccesses"]["gateway"]["fault"] = fault_info
        self.diagnosis["proccesses"]["gateway"]["gw_manager"] = gw_manager_info
        ...
        self.diagnosis["proccesses"]["gateway"]["apps"]["influx"] = \
            gw_apps["influx"]
        ssh_tunnel = await self.get_ssh_tunnel()
    ...
    async def send(self):
        ...
        rsp = await self.http_helper.request("Diagnosis", "POST", self.diagnosis_url, body)
        return rsp
